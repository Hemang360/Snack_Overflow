...
async function main() {
    try {
        ...
        const userIdentity = await wallet.get('labAgent-Rama');
        if (userIdentity) {
            console.log('An identity for the user "labAgent-Rama" already exists in the wallet');
            return;
        }
        ...
        const secret = await ca.register({
            ...
            enrollmentID: 'labAgent-Rama',
            role: 'client',
            attrs: [{ name: 'role', value: 'agent', ecert: true },{ name: 'uuid', value: 'labAgent-Rama', ecert: true }],
        }, adminUser);
        ...
        await wallet.put('labAgent-Rama', x509Identity);
        console.log('Successfully registered and enrolled labAdmin user "labAgent-Rama" and imported it into the wallet');

        // -----------------------Create Wallet with default balance on ledger------------------
                // Create a new gateway for connecting to our peer node.
                const gateway = new Gateway();
                await gateway.connect(ccp, { wallet, identity: 'labCompany01', discovery: { enabled: true, asLocalhost: true } });
                ...
                const args = {
                    agentId:"labAgent01",
                    labCompany:"labCompany01-XYZ",
                    name:"Rama",
                    city:"Amravati"
                }

                const res = await contract.submitTransaction('onboardLabAgent', JSON.stringify(args));
        ...
        console.error(`Failed to register user "labAgent-Rama": ${error}`);
        process.exit(1);
      }
}
