...
async function main() {
    try {
        ...
        const userIdentity = await wallet.get('Collector-ola');
        if (userIdentity) {
            console.log('An identity for the user "Collector-Rama04" already exists in the wallet');
            return;
        }
        ...
        const secret = await ca.register({
            ...
            enrollmentID: 'Collector-ola',
            role: 'client',
            attrs: [{ name: 'role', value: 'collector', ecert: true },{ name: 'uuid', value: 'Collector-Rama04', ecert: true }],
        }, adminUser);
        ...
        await wallet.put('Collector-ola', x509Identity);
        console.log('Successfully registered and enrolled cooperativeAdmin user "Collector-Rama04" and imported it into the wallet');

        // -----------------------Create Wallet with default balance on ledger------------------
                // Create a new gateway for connecting to our peer node.
                const gateway = new Gateway();
                await gateway.connect(ccp, { wallet, identity: 'Cooperative01', discovery: { enabled: true, asLocalhost: true } });
                ...
                // let collectorId="Collector-Rama04";
                // let cooperativeName="Cooperative01-ABC";
                // let name="Rama";
                // let city="Pune";

                const args = {
                    collectorId: "Collector-ola",
                    cooperativeName: "Cooperative01",
                    name: "Dr. Raj",
                    city: "Pune"
                };

                const res = await contract.submitTransaction('onboardCollector', JSON.stringify(args));
                console.log("/n === Onboard Collector success === /n", res.toString());
        ...
        console.error(`Failed to register user "Collector-sanji": ${error}`);
        process.exit(1);
      }
}
