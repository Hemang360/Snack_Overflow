...
async function main() {
    try {
        ...
        const userIdentity = await wallet.get('labCompany01');
        if (userIdentity) {
            console.log('An identity for the user "labCompany01" already exists in the wallet');
            return;
        }
        ...
        const secret = await ca.register({
            ...
            enrollmentID: 'labCompany01',
            role: 'client',
            attrs: [{ name: 'role', value: 'labAdmin', ecert: true },{ name: 'uuid', value: 'labCompany01', ecert: true }],
        }, adminUser);
        ...
        await wallet.put('labCompany01', x509Identity);
        console.log('Successfully registered and enrolled labAdmin user "labCompany01" and imported it into the wallet');
    } catch (error) {
        console.error(`Failed to register user "labCompany01": ${error}`);
        process.exit(1);
      }
}
